pipeline {
    ...
    environment {
        ...
        TOMCAT_URL = 'http://54.81.61.126:9090/manager/html'
        TOMCAT_USER = 'admin'
    ...
    stages {
        stage('Clone Repository') {
            steps {
                git url: 'https://github.com/SriVenkatG/Practical_3.git', branch: 'main'
            }
        }
        ...
        stage('Build Frontend (Vite)') {
            steps {
                dir("${env.FRONTEND_DIR}") {
                    script {
                        def nodeHome = tool name: 'NODEJS_HOME', type: 'jenkins.plugins.nodejs.tools.NodeJSInstallation'
    ...
    post {
        success {
            echo "✅ Backend deployed: http://54.81.61.126:9090/springapp1"
            echo "✅ Frontend deployed: http://54.81.61.126:9090/frontapp1"
        }
        failure {
